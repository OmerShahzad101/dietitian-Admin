...
import { beforeCoachMembership, createCoachMembership, listCoachMemberships, deleteCoachMembership } from './CoachMembership.action';
...
const CoachMembership = (props) => {
    ...
    }, [props.coachMemberships.coachMembershipListAuth, props.coachMemberships.coachMembershipList])
                            ...
                            <Col md="12">
                                <Card className="table-big-boy">
                                    <div className="d-block  d-sm-flex justify-content-between align-items-center register-users">
                                       ...
                                       { userAuthenticData?.permissionId?.coachMemberShipCreate ? <Button className="yellow-bg m-0">
                                            <span onClick={() => history.push(`/coachmembership/create/`)}>
                                                Coach Membership
                                            </span>
                                            <span className="pl-1">
                                            ...
                                            <Table className="table-striped w-full">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        {userAuthenticData?.permissionId?.coachMemberShipView||userAuthenticData?.permissionId?.coachMemberShipEdit||userAuthenticData?.permissionId?.coachMemberShipDelete ? <th className="td-actions ">ACTION</th> : ""}
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    {
                                                            ...
                                                            coachMemberships.map((data, index) => {
                                                                return (
                                                                    <tr key={index}>
                                                                        ...
                                                                        { data?.businessPractise ? <span className='lable-info'>All-In-One Business/Practice Platform</span> : ""}
                                                                       { data?.onBoardTraining ? <span className='lable-info'> Free onboarding training</span>: ""}
                                                                       { data?.healthMarketing ? <span className='lable-info'>Platform, marketing & health coaching training with certification</span>: ""}
                                                                       { data?.personalPlatform ? <span className='lable-info'>Personal platform link & page (like .healthiwealthi.io/ID)</span> : ""}
                                                                       { data?.onePageDashboard ? <span className='lable-info'>One page dashboard</span> : ""}
                                                                       { data?.clientAccessList ? <span className='lable-info'> Client access list (with icons to chat, talk or email each client easily) </span>: ""}
                                                                       { data?.HipaaSocial ? <span className='lable-info'> HIPAA compliant video/audio/chat (for personal consultations) </span>: ""}
                                                                       { data?.hipaaGroupCoaching ? <span className='lable-info'> HIPAA compliant group coaching video/audio/chat (for large groups) </span>: ""}
                                                                        {
                                                                            !(data?.["businessPractise"] || data?.["onBoardTraining"]
                                                                            || data?.["healthMarketing"] || data?.["personalPlatform"]
                                                                            || data?.["onePageDashboard"] || data?.["clientAccessList"]
                                                                            || data?.["HipaaSocial"] || data?.["hipaaGroupCoaching"])
                                                                            && "N/A" 
                                                                        }
                                                                        ...
                                                                        <td className="td-periods text-center">
                                                                            { data?.priceInCrypto ? data?.priceInCrypto  : "N/A" }
                                                                        </td>
                                                                        ...
                                                                        <td className="td-actions">
                                                                            ...
                                                                            {
                                                                                userAuthenticData?.permissionId?.coachMemberShipView === true ?
                                                                                <OverlayTrigger
                                                                                    overlay={
                                                                                ...
                                                                                >
                                                                                    <Button
                                                                                        ...
                                                                                        type="button"
                                                                                        variant="warning"
                                                                                        onClick={() => history.push(`/coachmembership/view/${data._id}`)}>
                                                                                        <i className="fas fa-eye"></i>
                                                                                    </Button>
                                                                                </OverlayTrigger> : ""
                                                                            }
                                                                            {
                                                                                userAuthenticData?.permissionId?.coachMemberShipEdit === true ?
                                                                                <OverlayTrigger
                                                                                    overlay={
                                                                                        <Tooltip id="tooltip-436082023">
                                                                                            Edit
                                                                                ...
                                                                                >
                                                                                    <Button
                                                                                        ...
                                                                                        type="button"
                                                                                        variant="warning"
                                                                                        onClick={() => history.push(`/coachmembership/edit/${data._id}`)}>
                                                                                        <i className="fas fa-edit"></i>
                                                                                    </Button>
                                                                                </OverlayTrigger> : ""
                                                                            }
                                                                            {
                                                                                userAuthenticData?.permissionId?.coachMemberShipDelete === true ?
                                                                                <OverlayTrigger
                                                                                    overlay={
            ...
            }
            {
                delModalCheck && <Modal show={delModalCheck} onHide={() => setDelModalCheck(false)}>
                    ...
                    <Modal.Footer className="d-flex justify-content-center">
                        ...
                        <Button variant="primary" onClick={delCoachMembership} className="yellow-bg save-btn">
                            Yes
                        </Button>
                    </Modal.Footer>
                </Modal>
...
export default connect(mapStateToProps, { beforeCoachMembership, createCoachMembership, listCoachMemberships, deleteCoachMembership })(CoachMembership)