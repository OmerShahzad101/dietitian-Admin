...
exports.edit = async (req, res, next) => {
  try {
    ...
    else if( payload.permissionId !== "" && payload.permissionId != undefined ){
      const admin = await Admin.findByIdAndUpdate(
...
exports.list = async (req, res, next) => {
  try {
    ...
    const totalAdmin = await Admin.aggregate([
      {
        $project: {
          _id: 1, username: 1, email: 1, phone: 1, address: 1, createdAt: 1, status: 1, type: 1,
          profileImage: 1
        }
      },
      { $match: filters },
  ...
  try {
    ...
    if (admin.length) {
      if (admin[0].resetCode === payload.code) {
        ...
        // let updatedAdmin = await Admin.findByIdAndUpdate({ _id: mongoose.Types.ObjectId(payload._id) }, { password: newPayload.password, resetCode: ''  }, { new: true })
        return res.send({
          success: true,
